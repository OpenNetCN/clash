...
| --- | --- | --- | --- | --- |
| 综合日常主力：AI + 流媒体 + 办公 | **BoostNet** | 200GB/月，¥49/月 | 优先官方客户端；新疆及境外不可用；不退款 | [看限制](#boostnet) · [查看套餐并注册](https://tw.boostqz.com/?path=register&code=kKMFirlm) |
...
| 年付轻量 / 家庭多设备 | **青云梯** | 60GB/月，¥96/年（折合 ¥8/月） | 此档仅年付；优先官方客户端，购买前确认设备支持 | [看限制](#qingyunti) · [查看套餐并注册](https://realvtest.qytvipaff.cc/register?aff=zcIlh0VY) |
| 补充专项出口 / IDE 登录场景 | **TNTCloud** | 试线路 110GB/月 ¥20；专项主用 220GB/月 ¥40 | 暂不支持通用客户端直接导入；出口需按目标平台验证 | [看限制](#tnt) · [查看套餐并注册](https://tanu095.tntvipaff.cc/#/register?code=7MPbfr89) |

<a id="client-tip"></a>

> **先确认设备，再付款：** WgetCloud 支持 Clash 等通用客户端；悠兔优先官方客户端，通用客户端以官网当前支持列表为准。BoostNet 以 Windows / macOS / Android 官方客户端为主。TNTCloud 暂不支持通用客户端直接导入；青云梯、银河云、唯兔云当前优先官方方案。**需要 iOS、Linux、路由器或自定义规则时，先向对应官网确认支持方式**，不能因为协议相同就套用其他机场的教程。

🧭 [按场景选](#person) · [七家选购摘要](#top7) · [节点观测](#daily-check) · [新手上手](#study) · [购买前检查](#risk)

📌 已有可用连接时，可访问 [gptvpnhelper.com 网页版](https://gptvpnhelper.com/) 查看完整测速图和长篇评测。
...
- **使用观察：** 品牌摘要说明适合场景与限制，完整截图见对应网页；单次测速只代表当时环境。
...
6. [网页版入口](#web-entry)
7. [新手上手：先确认设备，再注册连接](#study)
8. [客户端配置与订阅（GitHub 专区）](#config)：[客户端下载](#clients) · [Mihomo 配置](#clash-config) · [订阅导入](#client-config) · [术语表](#glossary)
9. [选购核对清单](#choose)
10. [什么是机场](#ssssr)
11. [购买建议与风险提醒](#risk)
12. [免责声明](#免责声明)
...
| 稳定性优先 / 团队办公 | [WgetCloud](#wgetcloud) | [BoostNet](#boostnet) | 支持通用客户端；基础档记录价 ¥237/季，200GB/30天；团队需核对在线 IP 与共享条款 |
...
**最近核对记录：** 季付套餐 2026-08-01；本次未复核结算页。
...
- **主要依据：** 支持通用客户端，有带日期的测速记录；品牌页 2026-08-01 资料记载已恢复季付。适合明确需要客户端灵活性并能接受季付实付门槛的人
...
**最近核对记录：** 未记录独立的套餐与客户端核对日期；本次未复核结算页。
...
| --- | --- | --- | --- | --- |
| **BoostNet** | AnyTLS / 三网直连 IEPL | 200GB/月，¥49/月 | Windows / macOS / Android 官方客户端 | 常规 5 台，企业 30 台；禁止共享；新疆及境外不可用；不退款 |
| **WgetCloud** | Trojan、SS / BGP + IEPL | 200GB/30天，记录价 ¥237/季（折合 ¥79/月） | 支持 Clash 等通用客户端 | 基础/优质/精品分别 3/4/5 个在线 IP，不能直接换算为设备数；注册需外网 |
| **悠兔** | Shadowsocks / IEPL + 多入口 | 试线路 150GB/月 ¥39；主力 300GB/月 ¥59 | 优先官方；通用客户端按支持列表 | 个人档 5 台、禁止共享；新疆、港澳台及境外不可用；封 22 端口 |
| **青云梯** | Shadowsocks / IPLC | 60GB/月，¥96/年 | 优先官方客户端 | 多设备，具体上限与共享权限查条款；轻量档仅年付 |
| **TNTCloud** | Trojan / IPLC | 试线路 110GB/月 ¥20；专项主用 220GB/月 ¥40 | 暂不支持通用客户端直接导入 | 资料标注不限设备，共享权限另查 |
| **唯兔云** | SS2022 / IPLC | 试线路 100GB/月 ¥14.9；主力 200GB/月 ¥29.9 | 优先官方方案 | 资料标注不限设备；目标地区与业务功能需分别验证 |
| **银河云** | Trojan / IEPL | 备用 50GB/月 ¥98/年；月付可看 100GB/月 ¥18 | 优先官方客户端 | 资料标注不限设备；退款条件需确认；轻量档仅年付 |
...
综合日常先看 [BoostNet](#boostnet) 的 200GB 月付档。必须使用 Clash 等通用客户端、能接受季付实付门槛时，看 [WgetCloud](#wgetcloud)；想先用月付验证 IEPL 与晚高峰视频表现，看 [悠兔](#youtu) 的 150GB 档，确认需求后再考虑 300GB 档。付款前先核对设备和地区限制。
...
[WgetCloud](#wgetcloud) 支持 Clash 等通用客户端；其他几家优先按官网当前说明使用官方客户端。iOS 上若机场明确支持 Nextin，可按其识别码或订阅指引接入；App 免费下载、含内购，下载地区和功能费用另行核对。需要自定义规则时再确认订阅格式和客户端版本是否兼容，详见 [客户端配置与订阅](#config)。
